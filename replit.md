...
- **Auto-Default Applicant Link Per Property**: Every property automatically has a shareable applicant link tied to its setup — landlords don't need to manually create one. Backend: idempotent `GET /api/rental/properties/:propertyId/default-link` (server/routes/rentalProperties.ts) get-or-creates a "default" unit (empty `unitLabel`, falls back to oldest if all labeled) plus its active link, using the property's `defaultCoverPageJson`, `defaultFieldSchemaJson`, `propertyTermsJson`, and unit `rentAmount`. A per-property in-process Promise lock (`defaultLinkLocks` Map) serializes parallel GETs so react-query refetch-on-focus can't race-create duplicates. Frontend: `PropertyCard` in `client/src/pages/properties.tsx` auto-fetches via `useQuery` and displays the URL prominently in a bordered box with Copy + Open buttons at the top of every card. The collapsible units section is renamed "Additional Unit(s)" and filtered to non-empty labels (the silent default unit is never shown as a confusing empty row). Same default-unit filter applied to `client/src/pages/rental-applications.tsx` for cross-page consistency; that page's empty-state copy directs landlords back to Properties to copy the auto-link, and "Add Unit" is positioned as optional for true multi-unit properties only. Existing properties get their link lazily created on first card render — no migration needed, existing manually-created empty-label units + links are reused (no duplicates, no broken URLs).
- **Per-Unit Security Deposit Override**: Each rental unit can override the property-level security deposit independently of rent. Schema: `rental_units.security_deposit_amount` (cents, nullable — null means "use property default"). Server: PATCH `/api/rental/properties/:propertyId/units/:unitId` accepts `securityDepositAmount`, syncs the value into the link's `mergedSchemaJson`. `server/routes/apply.ts` reads `liveUnit.securityDepositAmount` at request time (mirrors the live `rentAmount` pattern) so applicants always see the latest value. UI: `UnitCard` in properties.tsx has an inline edit row with "Use property deposit ($X)" / "Use property rent ($X)" copy buttons. Deposit badge hidden when null.
