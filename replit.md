...
- **Legislative Monitoring**: A safe, approval-gated two-job architecture ingests and normalizes legislative updates, queuing them for admin approval. Bills that are signed into law AND tagged high-relevance AND have at least one affected template are auto-approved at the bill level (template-level approval gate still applies). Set `AUTO_APPROVE_SIGNED_BILLS=false` to disable. Auto-approved bills are tagged with an "Auto" badge in the admin Reviewed Bills tab and `reviewed_by = 'system:auto-approve'`.
